<x-app-layout>
    <div class="container-fluid container-custom">
        ...
        <nav aria-label="breadcrumb">
            <ol class="breadcrumb rounded-0 shadow-sm p-2">
                <li class="breadcrumb-item"><a href="{{ route('dashboard') }}"
                        class="text-muted hover-text-primary transition-all duration-300">Inicio</a></li>
            ...
            <div class="flex items-center space-x-4">
                ...
                <div class="relative">
                    <select name="department_id" id="department_id"
                        class="select-department p-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 transition duration-300 ease-in-out w-80"
        ...
        @if(!isset($employees) || $employees->count() == 0)
            ...
            <div
                class="mt-6 p-6 bg-yellow-200 border-l-4 border-yellow-500 rounded-lg shadow-lg transition-all duration-500 ease-in-out transform hover:scale-105 hover:shadow-xl hover:bg-yellow-300">
                <div class="flex items-center">
                    <i class="fas fa-exclamation-triangle text-3xl text-yellow-500 mr-4"></i>
                    <div>
                        <h2 class="font-semibold text-xl text-gray-800">¡Atención! Es necesario seleccionar un departamento
                        </h2>
                        <p class="mt-2 text-gray-700">Para poder visualizar el cuadrante de horarios, es imprescindible
                            seleccionar un departamento de la lista desplegable. Si no se han asignado empleados a este
                            departamento, lamentablemente no se podrá mostrar ningún dato relacionado con los horarios de
                            trabajo.</p>
                        <p class="mt-2 text-gray-700">Si el departamento seleccionado no tiene empleados asignados o si aún
                            no se ha configurado el cuadrante de horarios, no será posible acceder a la información
                            relevante. Le recomendamos verificar que todos los departamentos cuenten con los empleados
                            necesarios y que se hayan configurado adecuadamente sus horarios.</p>
                        <p class="mt-2 text-gray-600 font-medium">Por favor, proceda seleccionando un departamento de la
                            lista para continuar con la visualización de los horarios.</p>
                    </div>
                </div>
            </div>
        @else
            ...
            <form action="{{ route('cuadrante.store') }}" method="POST">
                                                                    ...
                                                                    <td class="border">
                                                                        <div class="relative w-full">
                                                                            <select name="schedules[{{ $employee->id }}][{{ $day }}]"
                                                                                class="w-full p-3 border rounded-md focus:ring-2 focus:ring-blue-500 appearance-none">
                ...
                <button type="submit"
                    class="mt-6 bg-blue-600 text-white px-6 py-3 rounded-md shadow-lg transition duration-300 hover:bg-blue-700 focus:outline-none focus:ring-4 focus:ring-blue-500">
                    Guardar horarios
                </button>
            </form>
        @endif
